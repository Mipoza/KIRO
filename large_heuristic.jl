...
S_partition = get_large_station_partition(I)
T_partition = get_large_turbine_partition(I)

t_links = zeros(length(I.wind_turbines))
sub = SubStation[]
sub_cable = zeros(Int,length(I.substation_locations),length(I.substation_locations))

for i in 1:length(S_partition)
    Si = S_partition[i]
    Ti = T_partition[i]
